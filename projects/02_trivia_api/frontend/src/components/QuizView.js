...
class QuizView extends Component {
  ...
  componentDidMount(){
    $.ajax({
      url: `http://localhost:5000/categories`, //TODO: update request URL (DONE)
      type: "GET",
      success: (result) => {
  ...
  getNextQuestion = () => {
    ...
    $.ajax({
      url: 'http://localhost:5000/quizzes', //TODO: update request URL (DONE)
      type: "POST",
      dataType: 'json',
  ...
  evaluateAnswer = () => {
    // i had to change the implementation because there was some logical errors
    const formatGuess = this.state.guess.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,"").toLowerCase()
